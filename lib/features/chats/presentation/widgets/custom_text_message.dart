...
class CustomTextMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final backgroundColor = _resolveBackgroundColor(isSentByMe, theme, context);
    ...
    final textContent = _TextContent(
      message: message,
      textStyle: _isOnlyEmoji ? textStyle?.copyWith(fontSize: onlyEmojiFontSize) : textStyle,
    );
    ...
    return ClipRRect(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            if (linkPreviewWidget != null && linkPreviewPosition == LinkPreviewPosition.top) linkPreviewWidget,
            Container(
            ...
            if (linkPreviewWidget != null && linkPreviewPosition == LinkPreviewPosition.bottom) linkPreviewWidget,
          ],
        ),
      ),
    );
      ...
      case TimeAndStatusPosition.end:
        return Stack(
          children: [
            ...
            Positioned.directional(textDirection: textDirection, end: 0, bottom: 0, child: timeAndStatus),
          ],
        );
    }
  }

  Color? _resolveBackgroundColor(bool isSentByMe, _LocalTheme theme, BuildContext context) {
    if (isSentByMe) {
      return sentBackgroundColor ?? theme.primary;
    }
    return receivedBackgroundColor ?? Theme.of(context).colorScheme.surfaceContainerHighest;
...
class _TextContent extends StatefulWidget {
  const _TextContent({required this.message, required this.textStyle});
...
class _TextContentState extends State<_TextContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        //if (widget.showName) widget.nameBuilder?.call(widget.message.author) ?? UserName(author: widget.message.author),
        Text(!isShowOriginal ? widget.message.text : originalText, style: widget.textStyle),
        if (hasTranslated)
          !isShowOriginal
              ? GestureDetector(
                  onTap: () => setState(() => isShowOriginal = true),
                  child: Text(
                    loc.show_original,
                    style: Theme.of(
                      context,
                    ).textTheme.labelMedium!.copyWith(color: Theme.of(context).colorScheme.tertiary),
                  ),
                )
              : GestureDetector(
                  onTap: () => setState(() => isShowOriginal = false),
                  child: Text(
                    loc.return_translation,
                    style: Theme.of(
                      context,
                    ).textTheme.labelMedium!.copyWith(color: Theme.of(context).colorScheme.tertiary),
                  ),
